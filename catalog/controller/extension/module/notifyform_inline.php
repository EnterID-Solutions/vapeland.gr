<?php
class ControllerExtensionModuleNotifyformInline extends Controller {
	public function index() {
		...
		if (version_compare(VERSION,'2.2.0.0','<')){
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/module/'.$template_folder.'/notifyform_inline.tpl')) {
				return $this->load->view($this->config->get('config_template') . '/template/extension/module/'.$template_folder.'/notifyform_inline.tpl', $data);
			} else {
				return $this->load->view('default/template/extension/module/'.$template_folder.'/notifyform_inline.tpl', $data);
			}
		}else{
			return $this->load->view('extension/module/'.$template_folder.'/notifyform_inline', $data);
		}
	}
}