...
class ControllerModuleUniversalImport extends Controller {
  ...
  public function import_step2() {
    ...
    if ($this->request->post['import_source'] == 'ftp') {
      ...
      if (strpos($this->request->post['import_file'], '.google.com/') && !strpos($this->request->post['import_file'], 'export')) {
  ...
  public function process() {
    if (defined('GKD_CRON')) {
      ...
      if (is_file(DIR_APPLICATION . 'view/universal_import/profiles/' . $this->request->get['type'] . '/' . $_GET['profile'] . '.cfg')) {
        $config = include DIR_APPLICATION . 'view/universal_import/profiles/' . $this->request->get['type'] . '/' . $_GET['profile'] . '.cfg';
      } else {
        $this->tool->cron_log('Profile not found: '.DIR_APPLICATION . 'view/universal_import/profiles/' . $this->request->get['type'] . '/' . $_GET['profile'] . '.cfg');
        die('Profile not found: '.DIR_APPLICATION . 'view/universal_import/profiles/' . $this->request->get['type'] . '/' . $_GET['profile'] . '.cfg');
      }
  ...
  public function loadAPI($config, $import_file) {
    if (true) {
      ...
      if (!empty($config['import_auth_url'])) {
        try {
          $auth = $this->model_gkd_import_tool->callAPI($config['import_auth_url'], $config['import_auth'], false, false, 'POST');
        } catch (Exception $e) {
          echo json_encode(array('file_error' => $e->getMessage()));
          die;
        }

        $authRes = json_decode($auth);
      ...
      foreach ($commands as $apiUrl) {
        ...
        if (substr($apiUrl, 0, 4) == 'http') {
          ...
          try {
            $res = $this->model_gkd_import_tool->callAPI($apiUrl, $authorization, false, false, $method, $params, $headers);
          } catch (Exception $e) {
            echo json_encode(array('file_error' => $e->getMessage()));
            die;
          }
          ...
          while ($runCycle) {
            ...
            if (isset($reloadCallApi)) {
              try {
                $res = $this->model_gkd_import_tool->callAPI($currentApiUrl, $authorization, false, false, $method, $params, $headers);
              } catch (Exception $e) {
                echo json_encode(array('file_error' => $e->getMessage()));
                die;
              }
      ...
      try {
        $res = $this->model_gkd_import_tool->callAPI($fileUrl, $authorization, $import_file, false, 'GET', $params, $headers);
      } catch (Exception $e) {
        echo json_encode(array('file_error' => $e->getMessage()));
        die;
      }
      ...
      try {
        $res = $this->model_gkd_import_tool->callAPI($config['import_file'], html_entity_decode($config['import_auth'], ENT_QUOTES, 'UTF-8'), $import_file);
      } catch (Exception $e) {
        echo json_encode(array('file_error' => $e->getMessage()));
        die;
      }
