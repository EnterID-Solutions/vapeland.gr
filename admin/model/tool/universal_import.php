...
class ModelToolUniversalImport extends Model {
  ...
  public function process_product_update($config, $line) {
    ...
    if (!empty($config['option_identifier']) && substr($config['option_identifier'], 0, 1) == '_') {
    ...
    if ($mode == 'attribute_group') {
      ...
      if ($item_id) {
        if ($config['item_exists'] == 'update') {
          ...
          if (!$this->simulation) {
            $this->model_catalog_attribute->editAttribute($item_id, $data);
          }
          $data['row_status'] = 'updated';
          $this->session->data['obui_processed']['updated']++;
        } else if ($config['item_exists'] == 'soft_update') {
          ...
          if (!$this->simulation) {
            ...
            $this->load->model('gkd_import/attribute');
            $this->model_gkd_import_attribute->updateAttribute($item_id, $data);
          }
          $data['row_status'] = 'updated';
          $this->session->data['obui_processed']['updated']++;
        } else {
        ...
        if ($config['item_not_exists'] == 'insert') {
          if (!$this->simulation) {
            $this->load->model('gkd_import/attribute');
            $item_id = $this->model_gkd_import_attribute->addAttribute($data);
          }
          $data['row_status'] = 'inserted';
          $this->session->data['obui_processed']['inserted']++;
        } else {
        ...
        foreach ($extra_funcs as $func_type => &$func) {
          ...
          if (!isset($func['field']) || $func['field'] !== '') {
            ...
            // Web
              ...
              try {
                $res = $this->model_gkd_import_tool->callAPI($func['url'], $func['auth'], false, $func['path']);
              } catch (Exception $e) {
                $res = '';
            ...
            // HTML
              ...
              $line[$target] = @htmlspecialchars($init_value, ENT_QUOTES, 'UTF-8');
            } else if ($func_type == 'html_decode') {
              $line[$target] = @html_entity_decode($init_value, ENT_QUOTES, 'UTF-8');
        ...
        foreach ($jsonArray as $jsonKey => $jsonVal) {
          ...
          if ($jsonKey == $jsonPath[0] || $jsonPath[0] == '*') {
            ...
            if (isset($jsonPath[1])) {
              foreach ($jsonVal as $jsonKey1 => $jsonVal1) {
                if ($jsonKey1 == $jsonPath[1] || $jsonPath[1] == '*') {
                  ...
                  if (isset($jsonPath[2])) {
                        ...
                        if (isset($jsonPath[3])) {
                          foreach ($jsonVal2 as $jsonKey3 => $jsonVal3) {
                            if ($jsonKey3 == $jsonPath[3] || $jsonPath[3] == '*') {
                              ...
                              return $this->model_gkd_import_tool->arrayFlat($jsonVal3);
                            }
                          }
                        } else {
                          if (++$i < $this->session->data['obui_current_line']) { continue; }

                          return $this->model_gkd_import_tool->arrayFlat($jsonVal2);
                        }
                      }
                    }
                  } else {
                    ...
                    return $this->model_gkd_import_tool->arrayFlat($jsonVal1);
                  }
                }
              }
            } else {
              ...
              return $this->model_gkd_import_tool->arrayFlat($jsonVal);
            }
          }
        }
      } else {
