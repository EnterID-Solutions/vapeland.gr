...
class ModelGkdImportTool extends Model {
  ...
  public function urlify($value, $lang = null, $filename = false) {
    if (!empty($lang)) {
    ...
    if (!empty($filename)) {
      include_once(DIR_SYSTEM . 'library/gkd_urlify.php');
      return URLify::filter($value, 256, $lang, true);
    }
  ...
  // API Calls
  public function callAPI($url, $login = '', $copyPath = false, $getPath = false, $method = 'GET', $data = array(), $headers = array()) {
    $ch = curl_init();
    $debug = false;
  ...
  public function arrayFlat($array, $prefix = '', $level = 0) {
    $result = array();
    ...
    foreach ($array as $key => $value) {
      ...
      if (is_array($value)) {
        ...
        $result = array_merge($result, $this->arrayFlat($value, $new_key, ++$level));
      } else {
        $result[$new_key] = $value;
      }
    }
